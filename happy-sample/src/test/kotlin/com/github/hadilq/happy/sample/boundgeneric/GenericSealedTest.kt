...
sealed class A<T : Number> {
  @Happy
  data class HappyA<T : Number>(val a: T) : A<T>()
  class OptionOne<T : Number> : A<T>()
  data class OptionTwo<T : Number>(val one: T, val two: List<T>, val three: Int) : A<T>()
  data class OptionThree<T : Number>(val one: List<String>, val two: Alias, val three: List<String>) : A<T>()
  data class OptionFour<T : Number>(val one: Map<String, List<Int>>) : A<T>()
}
